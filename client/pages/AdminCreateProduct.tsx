...
interface SKUVariant {
  ...
  price: number;
  mrp: number;
  boxPrice: number; // quantity * price
  boxMrp: number; // quantity * mrp
}
...
export default function AdminCreateProduct() {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      console.log('Storage available:', !!client.storage);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (json?.success) {
        ...
        toast.error('Failed to create product: ' + (json?.error || 'Unknown error'));
      }
    } catch (err: any) {
      console.error(err);
      ...
      <main className="container max-w-3xl mx-auto py-8">
        ...
        <div className="mb-6 p-4 border rounded-lg bg-gray-50">
          ...
          <div className="mb-4">
            ...
            <p className="text-xs text-gray-500 mt-2">Upload multiple images (JPG, PNG, WebP). First image will be the primary image.</p>
          </div>
        ...
        {form.category && (
          <>
            ...
            <select className="w-full border rounded-md px-3 py-2 mb-4" value={form.subcategory} onChange={(e) => setForm({ ...form, subcategory: e.target.value })}>
        ...
        {/* SKU Variants Section - FIRST PRIORITY */}
        <div className="border-2 border-blue-500 rounded-lg p-6 bg-blue-50 mb-6">
          <h2 className="text-lg font-bold mb-2 text-blue-900">📦 SKU Variants * (Select Unit Type First)</h2>
          <p className="text-sm text-gray-700 mb-4">Start by adding product variants with Kg/Litre, quantities, and box packing</p>
                ...
                {skuVariants.map((variant) => (
                  <div key={variant.id} className="bg-white p-4 rounded-lg border border-blue-200">
                    ...
                    <div className="space-y-2">
                      ...
                      {/* Box Pricing - Prominent */}
                      <div className="pt-2 border-t-2 border-blue-200 bg-blue-50 -mx-4 -mb-4 px-4 py-3 rounded-b-lg">
                        <p className="text-xs text-gray-700 mb-1">📦 Box Package ({variant.piecesPerBox} units)</p>
                        <div className="flex justify-between items-center">
                          <div>
                            <p className="text-xs text-gray-600">Box Price</p>
                            <p className="font-bold text-lg text-blue-700">₹{variant.boxPrice.toFixed(2)}</p>
                          </div>
                          <div className="text-right">
                            <p className="text-xs text-gray-600">Box MRP</p>
                            <p className="font-semibold text-sm text-gray-700 line-through">₹{variant.boxMrp.toFixed(2)}</p>
                          </div>
                        </div>
                      </div>
                    </div>
        ...
        <Input type="number" value={form.stock_quantity} onChange={(e: any) => setForm({ ...form, stock_quantity: Number(e.target.value) })} className="mb-4" />

        <label className="block text-sm font-medium mb-1">Description</label>
        <textarea value={form.description} onChange={(e: any) => setForm({ ...form, description: e.target.value })} className="w-full border rounded-md p-3 mb-6" rows={5} />

        <div className="flex gap-3">
          <Button onClick={handleSubmit} disabled={uploading || form.images.length === 0 || skuVariants.length === 0}>
            {uploading ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" /> Uploading...</> : 'Create Product'}
          </Button>
